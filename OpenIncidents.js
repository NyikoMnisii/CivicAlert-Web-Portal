...
async function fetchReports() {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            // Filter reports based on status "Submitted" or "In progress" before displaying
            const filteredReports = Object.fromEntries(
                Object.entries(reports).filter(([_, report]) => report.status === 'Submitted' || report.status === 'In Progress')
            );
